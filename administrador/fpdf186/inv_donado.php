...
class PDF extends FPDF
{
    ...
    function Header(){
        ...
        $this->Cell(0, 6, utf8_decode('Inventario de Materiales Donados'), 0, 1, 'C'); // Agrega el titulo centrado
        ...
        $this->Cell($widths['nombre_donante'], 10, utf8_decode('Donante'), 1, 0, 'C', true);
...
$consulta = "SELECT articulos.id_articulo,articulos.nombre,articulos.descripcion, fecha_donados.cantidad,fecha_donados.nombre_donante,fecha_donados.fecha FROM articulos INNER JOIN fecha_donados ON articulos.id_articulo = fecha_donados.articulo_donado  WHERE articulos.estatus = 'activo' ORDER BY id_articulo";
...
while ($row = $resultado->fetch_assoc()) {
    ...
    $row['nombre_donante'] = ucwords($row['nombre_donante']);
