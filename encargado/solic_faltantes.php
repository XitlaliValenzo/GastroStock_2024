...
$sql_equipos = "SELECT s.equipo, GROUP_CONCAT(us.nombre, ' (', us.matricula, ')') AS alumnos, s.fecha_solicitud, s.fecha_reposicion, s.id, s.total
            FROM solicitud s
            INNER JOIN integrantes_equipo i ON s.equipo = i.num_equipo
            INNER JOIN usuarios us ON i.alumno = us.id
            WHERE s.estatus = 'faltantes'
            GROUP BY s.equipo";
...
if ($result_equipos->num_rows > 0) {
    while ($row_equipo = $result_equipos->fetch_assoc()) {
        ...
        if ($row_articulo) {
            ...
            echo "<td>
                      <a href='#' class='text-danger' data-toggle='modal' data-target='#modal_articulo" . $row_articulo['id'] . "'>Ver material</a>
                  </td>";
            ...
            echo "<td colspan='2'>No se encontraron artículos para esta solicitud</td>";
        }

        echo "<td>".$row_equipo['fecha_solicitud']."</td>
              <td>".$row_equipo['fecha_reposicion']."</td>
    ...
    <div class='modal-dialog' role='document'>
        <div class='modal-content'>
            ...
            <div class='modal-body'>
                ...
                <form class='cambiar-estatus-form' method='POST' action='actualizar_estatus_faltantes.php'>
