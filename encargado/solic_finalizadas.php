...
$sql_equipos = "SELECT s.equipo, GROUP_CONCAT(us.nombre, ' (', us.matricula, ')') AS alumnos, s.fecha_solicitud, s.fecha_entrega, s.id, s.observaciones
            FROM solicitud s
            INNER JOIN integrantes_equipo i ON s.equipo = i.num_equipo
            INNER JOIN usuarios us ON i.alumno = us.id
            WHERE s.estatus = 'finalizada'
            GROUP BY s.equipo";
...
if ($result_equipos->num_rows > 0) {
    while ($row_equipo = $result_equipos->fetch_assoc()) {
        ...
        echo "<td>".$row_equipo['fecha_solicitud']."</td>
              <td>".$row_equipo['fecha_entrega']."</td>
              <td>".$row_equipo['observaciones']."</td>
              <td>
