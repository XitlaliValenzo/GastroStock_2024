...
if($_FILES['rutaImagen']){
		...
		if(in_array($fileExtension, $allowedfileExtensions) && $fileSize <= $maxFileSize) {
			...
			if(move_uploaded_file($fileTmpPath, $dest_path)) {
				...
				if($stmt = $con->prepare($sql)) {
					...
					if($stmt->execute()) {
						header("Location: ../inv_danado.php?m=2");
					} else {
				...
				header("Location: ../inv_danado.php?m=3");
			}
		} else {
			if ($fileSize > $maxFileSize) {
				header("Location: ../inv_danado.php?m=4");
			} else {
				header("Location: ../inv_danado.php?m=5");
			}
		}
	} else {
	...
	header("Location: ../inv_danado.php?m=6");
}
