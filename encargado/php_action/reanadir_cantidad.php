<?php 
	...
	function actualizarPerdidos($con,$cantidad_total_perdidos,$id_articulo){
	...
	if ($_POST) {
		...
		$cantidad_total_perdidos = $cantidad_stock - $cantidad_agregar;
		actualizarPerdidos($con,$cantidad_total_perdidos,$id_articulo);
		...
		if ($tipo == 'donativo') {
			...
			if($con->query($sql3)){
				if ($con->query($sql4)) {
					header("Location: ../inv_perdido.php?m=13");
				}else{
